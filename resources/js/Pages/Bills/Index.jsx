...
export default function BillsIndex({ bills, vendors }) {
                ...
                {billList.length === 0 ? (
                  ...
                  billList.map((b) => (
                    <tr key={b.id} className="hover:bg-slate-50/50">
                      <td className="py-2.5 px-3 font-mono font-bold text-amber-600">{b.bill_number}</td>
                      <td className="py-2.5 px-3 font-semibold text-slate-900">{b.contact?.name}</td>
                      <td className="py-2.5 px-3 text-slate-500">{new Date(b.bill_date).toLocaleDateString("en-GB")}</td>
                      <td className="py-2.5 px-3">
                        <span className="px-2 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-800 rounded">
                          {b.status}
                        </span>
                      </td>
                      <td className="py-2.5 px-3 text-right font-bold text-slate-900">{formatBDT(b.total_amount)}</td>
                    </tr>
                  ))
                )}
              </tbody>
